...
\begin{enumerate}
    \item 设CS=2500H，DS=2400H，SS=2430H，BP=0200H，SI=0010H，DI=0206H，请分别写出下述x86指令的源操作数的物理地址。
    \begin{enumerate}
        \item MOV AX，[2000H]
        \item MOV AX，[BP+SI+4]
        \item MOV AX，[DI+100H]
...
\textbf{解答：}
    \begin{enumerate}
        \item 分支条件成立时的目标地址为0x40000044，如果不成立则目标地址为0x40000024。
        \item 0x400C03C0
        \item 不能。因为j指令的目标地址由当前PC的高4位和指令中的26位立即数组成，而当前PC的高四位为0x40，无法直接跳转到0x30开头的地址空间。
    \end{enumerate}
\end{document}
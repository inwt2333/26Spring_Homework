...
\begin{problem}
【多选】关于行波进位加法器和超前进位加法器各自的优缺点描述正确的是（AD）
\begin{itemize}
...
\begin{problem}
【多选】对于经过流程优化、面积优化后的32-bit乘法器（课件第33页），需要包含以下哪些组成部分？（DEF）
\begin{itemize}
    \item[A ：] 64位ALU 
    \item[B ：] 32位的乘数寄存器 
    \item[C ：] 64位的被乘数寄存器 
...
\begin{solution}
\textcircled{1} = 1, \textcircled{2} = 0, \textcircled{3} = 1, \textcircled{4} = 0, \textcircled{5} = 0, \textcircled{6} = 1。

当输入S和R同时为0时，该SR锁存器的输入是非法的。
\end{solution}

\begin{problem}
【问答题】已知全加器由两个半加器构成，请通过编号说明下述全加器中每个半加器是如何组成的，并写出\textcircled{6}和\textcircled{7}分别对应什么输出。
...
\begin{solution}
\textcircled{1}和\textcircled{3}组成一个半加器，其中\textcircled{1}输出的是A和B的和，\textcircled{3}输出的是A和B的进位。

\textcircled{2}和\textcircled{4}组成另一个半加器，其中\textcircled{4}输出的是\textcircled{1}的输出和$\text{C}_\text{in}$的和，\textcircled{2}输出的是\textcircled{1}的输出和$\text{C}_\text{in}$的进位。

\textcircled{5}将上面两步的进位汇总，因此\textcircled{6}输出的是全加器的进位，而\textcircled{7}输出的是全加器的和。
\end{solution}
...
轮数 & 乘积寄存器 & 被乘数寄存器 & 乘数寄存器 \\
\hline
初始 & 00000000 & 00000010 & 0011\\
1 & 00000010 & 00000100 & 0001\\
2 & 00000110 & 00001000 & 0000\\
3 & 00000110 & 00010000 & 0000\\
4 & 00000110 & 00100000 & 0000\\
\end{tabular}
\end{center}
...
轮数 & 除数寄存器 & 商寄存器 & 余数寄存器 \\
\hline
初始 & 00100000 & 0000 & 00000111 \\
1 & 00010000 & 0000 & 00000111 \\
2 & 00001000 & 0000 & 00000111 \\
3 & 00000100 & 0000 & 00000111 \\
4 & 00000010 & 0001 & 00000011 \\
5 & 00000001 & 0011 & 00000001 \\
\end{tabular}
\end{center}

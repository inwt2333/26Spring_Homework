...
\begin{problem}
假设内存中存储的字节内容如下， \verb|0x100| 和 \verb|0x104| 起分别对应两个32位有符号整型数，请分别写出在大端序和小端序情况下，这两个数相加和相乘的结果（用十六进制表示）。
\begin{center}
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        地址 & \verb|0x0100| & \verb|0x0101| & \verb|0x0111| & \verb|0x1000| \\
        \hline
    ...
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        地址 & \verb|0x1001| & \verb|0x1010| & \verb|0x1011| & \verb|0x1100| \\
        \hline
...
\end{problem}
\begin{note}
怀疑是题目错误？因为这8个地址在内存中不是连续的，此处当作连续的 \texttt{0x0100} 到 \texttt{0x0107}。
\end{note}
\begin{solution}

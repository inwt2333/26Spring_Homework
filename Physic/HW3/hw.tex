...
(3) 由于 $\langle \hat{p}_x^2 \rangle = 2m\langle \hat{H} \rangle = \frac{10\hbar^2}{a^2}$，且 $\bar{p}_x = 0$，所以 $\Delta p_x = \sqrt{\langle \hat{p}_x^2 \rangle} = \frac{\sqrt{10}\hbar}{a}$。
...
所以 $t$ 时刻的波函数为 $\psi(x, t) = \sum_{n=1,3,5\dots}^\infty \frac{8\sqrt{15}}{n^3 \pi^3} \sqrt{\frac{2}{a}} \sin\left(\frac{n\pi x}{a}\right) e^{-in^2\pi^2\hbar t/2ma^2 }$
\end{solution}
...
将波函数展开为 $\hat{L}_z$ 在 $\phi$ 表象中的本征态 $\Phi_m(\phi) = \frac{1}{\sqrt{2\pi}} e^{im\phi}$：
...
&= \frac{1}{4}\hbar\omega + \frac{3}{10}\hbar\omega + \frac{3}{4}\hbar\omega = \frac{13}{10}\hbar\omega
\end{align*}
\end{solution}
\end{document}
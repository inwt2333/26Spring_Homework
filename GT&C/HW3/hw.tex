...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        此时，我们观察边集 $E' = \{e_1, e_2\}$。由于\{$u, v$\}为点割集，因此所有连接 $C_1$到$C_2$的边，必须经过$u,v$其一，若经过$u$则必须经过$e_1$，若经过$v$则必须经过$e_2$。且$u,v$之间不连通，因此若在原图 $G$ 中切断 $e_1$ 和 $e_2$，所有从$C_1$到$C_2$的边都将被切断，图将被分为完全不连通的两部分：一部分包含 $V(C_1) \cup \{u\}$，另一部分包含 $V(C_2) \cup \{v\}$。这说明存在大小为 2 的边割集，即 $\kappa'(G) \leqslant 2$。这再次与假设 $2 < \kappa'(G)$ 矛盾。
    \end{itemize}
\end{enumerate}
